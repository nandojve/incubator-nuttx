...
int board_boot_image(FAR const struct boardioc_boot_info_s *info)
{
  ...
  fd = open(info->path, O_RDONLY | O_CLOEXEC);
  if (fd < 0)
    {
      syslog(LOG_ERR, "Failed to open %s with: %d", info->path, fd);
      return fd;
    }

  bytes = pread(fd, &vt, sizeof(vt), info->header_size);
  if (bytes != sizeof(vt))
    {
...
//                    info->header_size, (uint32_t) vt);
  ...
  syslog(LOG_INFO, " - flash base:   0x%08x\n", info->flash_base);
  syslog(LOG_INFO, " - image offset: 0x%08x\n", info->image_offset);
  syslog(LOG_INFO, " - header size:  0x%08x\n", info->header_size);
